...
def _create_chatbot_node_with_llm(llm: BaseChatModel):
    """Create a chatbot node that uses the provided LLM instead of global config."""
    def chatbot_with_llm(state: TutorState):
        messages = state['messages']
        context_text = state.get('context_text', '')
        target_language = state.get('target_language', 'Chinese')
    ...
    return chatbot_with_llm


def create_tutor_graph_with_llm(llm: BaseChatModel):
    """
    Create a tutor graph with an externally provided LLM.

    Use this when the user has configured their own LLM provider.
    ...
    workflow.add_node("chatbot", _create_chatbot_node_with_llm(llm))
    workflow.set_entry_point("chatbot")
    workflow.add_edge("chatbot", END)
